...
func registerPeer(w http.ResponseWriter, r *http.Request) {
	...
	// 체인 정체성 확인: 제네시스 gov_id와 일치해야 가입 허용
	blk0, err := getBlockByIndex(0)
	if err != nil || blk0.GovID != req.GovID {
		http.Error(w, "Gov_id mismatch", http.StatusForbidden)
		return
	}
...
// 3) 선출된 부트노드는 다른 Gov노드들에게 자신의 주소를 전파
// 4) 선출된 부트노드는 Hos 부트노드들에게 자신의 주소를 전파
...
func electAndSwitch() {
	...
	// 자신이 승자노드가 된 경우, 다른 Gov 노드들과 hos 부트노드들에게 자신의 주소 전파
	if winner.Addr == self {
		isBoot.Store(true)
		setBootAddr(self)
		broadcastNewBoot(self) // 다른 Gov 노드들에게 전파
		broadcastNewBootToHos(self)
...
// 자신이 새 Gov 부트노드로 선출되었을 때, 기존에 등록된 모든 Hos 부트노드들에게 전파
func broadcastNewBootToHos(newBoot string) {
	for hosID, hosBoot := range hosBootMap {
		go func(id, dst string) {
			log.Printf("[BOOT][ToHos] New Gov Boot Node's Addr is now sending to : %s", dst)
			body, _ := json.Marshal(map[string]string{"gov_boot": newBoot})
			_, err := http.Post("http://"+dst+"/chgGovBoot", "application/json", strings.NewReader(string(body)))
			if err != nil {
				log.Printf("[BOOT] notify failed to %s: %v", dst, err)
			}
		}(hosID, hosBoot)
	}
	log.Printf("[BOOT][GovtoHos] New Gov Boot Node's Addr was sent to Hos Boot Nodes")
}

// 신규 Hos체인의 부트노드가 앵커를 제출했을 때, 이를 저장한 후 다른 gov 노드에게 전파
func broadcastNewHosBoot(hosID, hosBoot string) {
	// 부트노드 자신에게 신규 hos 부트노드 주소 저장
	logInfo("[BOOT] Store newHosBoot to HosBootMap")
	setHosBootAddr(hosID, hosBoot)
	// 나머지 gov 노드들에게 hos 부트노드 주소 전파
	for _, peer := range peersSnapshot() {
		go func(dst string) {
			body, _ := json.Marshal(map[string]string{"hos_id": hosID, "hos_boot": hosBoot})
			logInfo("[BOOT] notify new hosBoot to %s", dst)
			_, err := http.Post("http://"+dst+"/hosBootNotify", "application/json", strings.NewReader(string(body)))
			if err != nil {
				log.Printf("[BOOT] notify failed to %s: %v", dst, err)
			}
		}(peer)
	}
	log.Printf("[BOOT][NETWORK] Complete Broadcasting New Hos Boot : %s", hosBoot)
}

// 신규 Hos의 HosID 및 부트노드 주소 수신(gov 모든 노드 수행)
// POST : /hosBootNotify
func hosBootNotify(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "method not allowed", 405)
		return
	}
	...
	if json.NewDecoder(r.Body).Decode(&in) != nil || in.HosBoot == "" {
		http.Error(w, "bad body", 400)
		return
	}
	// 전달받은 부트노드 주소가 실제로 살아있는지 검증
	if _, ok := probeStatus(in.HosBoot); !ok {
		http.Error(w, "boot not reachable", 502)
		log.Printf("[BOOT] received new boot addr (%s) but not reachable", in.HosBoot)
		return
	}
	// Hos 체인의 ID와 부트노드 주소 저장
	setHosBootAddr(in.HosID, in.HosBoot)
	// 성공 로그 출력
	log.Printf("[BOOT] Successfully received hosBoot: %s : %s to HosBootMap )", in.HosID, in.HosBoot)
	w.WriteHeader(200)
}
...
func setHosBootAddr(hosID, addr string) {
	hosBootMapMu.Lock()
	hosBootMap[hosID] = addr
	hosBootMapMu.Unlock()
	logInfo("[BOOT] set new HosBoot addr to HosBootMap: %s", addr)
}
func getHosBootAddr(hosID string) string {
	hosBootMapMu.RLock()
	defer hosBootMapMu.RUnlock()
	return hosBootMap[hosID]
}
