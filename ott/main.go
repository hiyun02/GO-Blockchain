...
func main() {
	...
	dbPath := getEnvDefault("OTT_DB_PATH", "blockchain_db")
	ottID := getEnvDefault("OTT_ID", "OTT-A")
	addr := getEnvDefault("PORT", "5000")
	addr = ":" + addr

	boot = getEnvDefault("BOOTSTRAP_ADDR", "ott-boot:5000") // 부트노드 고정주소
	self = getEnvDefault("NODE_ADDR", "ott-node-00:5000")   // 이 노드의 외부접속 주소
